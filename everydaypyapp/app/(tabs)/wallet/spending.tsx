...
export default function SpendingScreen() {
  ...
  const totalSpending = demoRegistry.getTotalSpending();
  const spendingWallets = demoRegistry.listByStatus(WalletStatus.SPENDING);

  return (
    <View style={[walletStyles.basicContainer]} key={`spending-${dataVersion}`}>
      {refreshing && (
      ...
      <View className="OverviewContainer" style={[walletStyles.verticalContainer, {margin: 30}]}>
        ...
        <Text style={walletStyles.funds}>${totalSpending.toFixed(2)}</Text>
      ...
      <View style={walletStyles.twoColRightFirst}>
        {spendingWallets.map((w) => {
          ...
          return (
            <View className="SpendingWallet" key={w.getName()} style={walletStyles.OverviewCard}>
        ...
        <View key={"add-wallet-spending"} style={walletStyles.OverviewCard}>
